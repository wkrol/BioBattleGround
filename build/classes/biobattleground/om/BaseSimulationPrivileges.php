 ...
 */
abstract class BaseSimulationPrivileges extends BaseObject  implements Persistent
{
  ...
  const PEER = 'SimulationPrivilegesPeer';
	 ...
	 * @return     SimulationPrivileges The current object (for fluent API support)
	 */
	...
	public function setIdUser($v)
	{
		...
		if ($this->id_user !== $v) {
			$this->id_user = $v;
			$this->modifiedColumns[] = SimulationPrivilegesPeer::ID_USER;
		}
	...
	public function setCreate($v)
	{
		...
		if ($this->create !== $v) {
			$this->create = $v;
			$this->modifiedColumns[] = SimulationPrivilegesPeer::CREATE;
		}
	...
	public function setJoin($v)
	{
		...
		if ($this->join !== $v) {
			$this->join = $v;
			$this->modifiedColumns[] = SimulationPrivilegesPeer::JOIN;
		}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->create = ($row[$startcol + 2] !== null) ? (int) $row[$startcol + 2] : null;
			$this->join = ($row[$startcol + 3] !== null) ? (int) $row[$startcol + 3] : null;
			$this->resetModified();
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		$stmt = SimulationPrivilegesPeer::doSelectStmt($this->buildPkeyCriteria(), $con);
	...
	public function save(PropelPDO $con = null)
	{
		...
		try {
			...
			if ($ret) {
				...
				$this->postSave($con);
				SimulationPrivilegesPeer::addInstanceToPool($this);
			} else {
				$affectedRows = 0;
			}
			$con->commit();
	...
	protected function doValidate($columns = null)
	{
		if (!$this->alreadyInValidation) {
			...
			if (($retval = SimulationPrivilegesPeer::doValidate($this, $columns)) !== true) {
	...
	{
		$pos = SimulationPrivilegesPeer::translateFieldName($name, $type, BasePeer::TYPE_NUM);
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		if (array_key_exists($keys[3], $arr)) $this->setJoin($arr[$keys[3]]);
	}
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(SimulationPrivilegesPeer::ID)) $criteria->add(SimulationPrivilegesPeer::ID, $this->id);
		if ($this->isColumnModified(SimulationPrivilegesPeer::ID_USER)) $criteria->add(SimulationPrivilegesPeer::ID_USER, $this->id_user);
		if ($this->isColumnModified(SimulationPrivilegesPeer::CREATE)) $criteria->add(SimulationPrivilegesPeer::CREATE, $this->create);
		if ($this->isColumnModified(SimulationPrivilegesPeer::JOIN)) $criteria->add(SimulationPrivilegesPeer::JOIN, $this->join);
	...
	{
		$criteria = new Criteria(SimulationPrivilegesPeer::DATABASE_NAME);
		$criteria->add(SimulationPrivilegesPeer::ID, $this->id);
	 ...
	 *
	 * @param      boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
	 * @return     SimulationPrivileges Clone of current object.
	 * @throws     PropelException
	 */
	 ...
	 *
	 * @return     SimulationPrivilegesPeer
	 */
	public function getPeer()
	{
		if (self::$peer === null) {
			self::$peer = new SimulationPrivilegesPeer();
		}
		return self::$peer;
	}
	...
	public function setUser(User $v = null)
	{
		...
		if ($v !== null) {
			$v->addSimulationPrivileges($this);
		}
...
abstract class BaseSimulationPrivileges extends BaseObject  implements Persistent {
	...
	public function delete(PropelPDO $con = null)
	{
		...
		try {
			$ret = $this->preDelete($con);
			if ($ret) {
				SimulationPrivilegesPeer::doDelete($this, $con);
				$this->postDelete($con);
				$this->setDeleted(true);
				$con->commit();
			} else {
	...
	protected function doSave(PropelPDO $con)
	{
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					$pk = SimulationPrivilegesPeer::doInsert($this, $con);
