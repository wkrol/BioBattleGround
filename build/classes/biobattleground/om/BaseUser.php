...
abstract class BaseUser extends BaseObject  implements Persistent
{
	...
	/**
	 * Clears out the collSimulationPrivilegess collection
	 *
	 * This does not modify the database; however, it will remove any associated objects, causing
	 * them to be refetched by subsequent calls to accessor method.
	 *
	...
	/**
	 * Initializes the collSimulationPrivilegess collection.
	 *
	 ...
	 * @param      Criteria $criteria optional Criteria object to narrow the query
	 * @param      PropelPDO $con optional connection object
	 * @return     PropelCollection|array SimulationPrivileges[] List of SimulationPrivileges objects
	 * @throws     PropelException
	 */
	public function getSimulationPrivilegess($criteria = null, PropelPDO $con = null)
	{
		if(null === $this->collSimulationPrivilegess || null !== $criteria) {
			if ($this->isNew() && null === $this->collSimulationPrivilegess) {
				// return empty collection
				$this->initSimulationPrivilegess();
			} else {
				$collSimulationPrivilegess = SimulationPrivilegesQuery::create(null, $criteria)
					->filterByUser($this)
					->find($con);
				if (null !== $criteria) {
					return $collSimulationPrivilegess;
				}
				$this->collSimulationPrivilegess = $collSimulationPrivilegess;
			}
		}
	...
	public function countSimulationPrivilegess(Criteria $criteria = null, $distinct = false, PropelPDO $con = null)
	{
		if(null === $this->collSimulationPrivilegess || null !== $criteria) {
			if ($this->isNew() && null === $this->collSimulationPrivilegess) {
				...
				$query = SimulationPrivilegesQuery::create(null, $criteria);
				if($distinct) {
					$query->distinct();
				}
				return $query
			...
			return count($this->collSimulationPrivilegess);
		}
	}
